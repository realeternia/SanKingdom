...
namespace CommonConfig
{
    public class TechSkillConfig
    {
        ...
        public string Des;
        /// <summary>
        ///大类：Dev / KingAction / Arms
        /// </summary>
        public string Category;
        /// <summary>
        ///作用目标：Dev的CityDevConfig ID / KingAction的DevConfig ID / Arms的ArmsConfig ID
        /// </summary>
        public int Target;
        /// <summary>
        ...
        public static void Load()
        {
            ...
            // ============================================================
            // Dev 行动强化模板（31001-31099）
            // Target=CityDevConfig ID
            // EnhanceType: AmountAdd(产出加算) / AmountMul(产出乘算) / CostReduce(消耗降低) / SlotAdd(槽位增加)
            // ============================================================
            ...
            // ============================================================
            // KingAction 君令强化模板（31201-31299）
            // Target=CityDevConfig ID（君令对应的devId）
            // 每个君令补齐：效果/成功率/消耗/槽位
            // ============================================================

            // --- 走访(21202) ---
            config[31201] = new TechSkillConfig(31201, "走访成功率", "走访成功率+x%", "KingAction", 21202, "SuccessMul", "rate");
            config[31202] = new TechSkillConfig(31202, "走访消耗", "走访金钱消耗-x%", "KingAction", 21202, "CostReduce", "gold");
            config[31203] = new TechSkillConfig(31203, "走访槽位", "走访人数+x", "KingAction", 21202, "SlotAdd", "slot");

            // --- 交易(21203) ---
            config[31211] = new TechSkillConfig(31211, "交易汇率", "交易汇率+x%", "KingAction", 21203, "AmountMul", "rate");
            config[31212] = new TechSkillConfig(31212, "交易消耗", "交易金钱消耗-x%", "KingAction", 21203, "CostReduce", "gold");
            config[31213] = new TechSkillConfig(31213, "交易槽位", "交易人数+x", "KingAction", 21203, "SlotAdd", "slot");

            // --- 登用(21204) ---
            config[31221] = new TechSkillConfig(31221, "登用成功率", "登用成功率+x%", "KingAction", 21204, "SuccessMul", "rate");
            config[31222] = new TechSkillConfig(31222, "登用消耗", "登用金钱消耗-x%", "KingAction", 21204, "CostReduce", "gold");
            config[31223] = new TechSkillConfig(31223, "登用槽位", "登用人数+x", "KingAction", 21204, "SlotAdd", "slot");

            // --- 褒奖(21205) ---
            config[31231] = new TechSkillConfig(31231, "褒奖效果", "褒奖忠心效果+x%", "KingAction", 21205, "AmountMul", "loyalty");
            config[31232] = new TechSkillConfig(31232, "褒奖消耗", "褒奖金钱消耗-x%", "KingAction", 21205, "CostReduce", "gold");
            config[31233] = new TechSkillConfig(31233, "褒奖槽位", "褒奖人数+x", "KingAction", 21205, "SlotAdd", "slot");

            // --- 奖赏(21206) ---
            config[31241] = new TechSkillConfig(31241, "奖赏效果", "奖赏忠心效果+x%", "KingAction", 21206, "AmountMul", "loyalty");
            config[31242] = new TechSkillConfig(31242, "奖赏消耗", "奖赏金钱消耗-x%", "KingAction", 21206, "CostReduce", "gold");
            config[31243] = new TechSkillConfig(31243, "奖赏槽位", "奖赏人数+x", "KingAction", 21206, "SlotAdd", "slot");

            // --- 破坏(21207) ---
            config[31251] = new TechSkillConfig(31251, "破坏效果", "破坏城防效果+x%", "KingAction", 21207, "AmountMul", "wall");
            config[31252] = new TechSkillConfig(31252, "破坏成功率", "破坏成功率+x%", "KingAction", 21207, "SuccessMul", "rate");
            config[31253] = new TechSkillConfig(31253, "破坏槽位", "破坏人数+x", "KingAction", 21207, "SlotAdd", "slot");

            // --- 扰乱(21208) ---
            config[31261] = new TechSkillConfig(31261, "扰乱效果", "扰乱民心效果+x%", "KingAction", 21208, "AmountMul", "happy");
            config[31262] = new TechSkillConfig(31262, "扰乱成功率", "扰乱成功率+x%", "KingAction", 21208, "SuccessMul", "rate");
            config[31263] = new TechSkillConfig(31263, "扰乱槽位", "扰乱人数+x", "KingAction", 21208, "SlotAdd", "slot");

            // --- 外交(21209) ---
            config[31271] = new TechSkillConfig(31271, "外交效果", "亲善效果+x%", "KingAction", 21209, "AmountMul", "relation");
            config[31272] = new TechSkillConfig(31272, "外交消耗", "外交金钱消耗-x%", "KingAction", 21209, "CostReduce", "gold");
            config[31273] = new TechSkillConfig(31273, "外交槽位", "外交人数+x", "KingAction", 21209, "SlotAdd", "slot");

            // --- 挑拨(21210) ---
            config[31281] = new TechSkillConfig(31281, "挑拨效果", "挑拨效果+x%", "KingAction", 21210, "AmountMul", "relation");
            config[31282] = new TechSkillConfig(31282, "挑拨成功率", "挑拨成功率+x%", "KingAction", 21210, "SuccessMul", "rate");
            config[31283] = new TechSkillConfig(31283, "挑拨槽位", "挑拨人数+x", "KingAction", 21210, "SlotAdd", "slot");

            // --- 移动(21102) ---
            config[31291] = new TechSkillConfig(31291, "移动人数", "单次移动英雄数+x", "KingAction", 21102, "SlotAdd", "slot");
            config[31292] = new TechSkillConfig(31292, "英雄移动效率", "英雄移动效率+x%", "KingAction", 21102, "AmountMul", "heroMove");
            config[31293] = new TechSkillConfig(31293, "部队移动效率", "部队移动效率+x%", "KingAction", 21102, "AmountMul", "armyMove");

            // ============================================================
            // Arms 兵种强化模板（31301-31399）
            // Target=ArmsConfig ID
            // EnhanceType: ArmsAttrAdd(兵种属性加算)
            // ============================================================
