...
public class SkillAttackedShadow : Skill
{
    ...
    public override void OnAttacked(Chess attacker, string damType, int damage)
    {
        if (count > 0 && CheckBurst(attacker))
        {
            ...
            var shadowUnit = BattleManager.Instance.SpawnUnitsForRegion(owner.GetPlayerInfo(), 501002, randomPosition, owner.forceId, HeroConfig.GetConfig(owner.heroId).Icon);
